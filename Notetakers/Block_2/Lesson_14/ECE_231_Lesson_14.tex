...
\begin{enumerate}
	...
	\item OpAmps have very high gain. Theoretically, an open loop gain is $\infty$ but typical real values are 20k - 200k.
\end{enumerate}
}
...
V_{out} = V_{R1} + V_{R2}
\end{equation}
which equals (voltage divider rearranged)
\begin{equation}
...
\begin{equation}
K=\frac{R_1+R_2}{R_2}=\frac{V_o}{V_i}
\end{equation}
...
\begin{equation}
\frac{V_{out}}{V_{in}}=\frac{-R_2}{R_1}=K
\end{equation}
}
...
\begin{table}[h]
	...
	\begin{tabular}{|l|c|c|}
		\hline
		Type & K & Account for input loading \\
		\hline \hline
		Non-inverting & $\frac{R_1+R_2}{R_2}$ & No \\
		Inverting     & $\frac{-R_2}{R_1}$    & Yes \\
		\hline
	\end{tabular}
	\caption{Op Amp Characteristics}
