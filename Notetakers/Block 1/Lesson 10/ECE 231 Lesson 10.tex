...
Start by wrting an equation for the voltage across the load resistor:
\soln{1in}{
\begin{equation}
v_{Load}=\frac{R_L}{R_L+R_T}V_T
\end{equation}
...
\textbf{Now found the value for $R_T$ that give max power, what is the maximum power delivered?}
\soln{2in}{
Plug $R_L=R_T$ into the power equation and solve to get
\begin{equation}
P_{Load}=\frac{R_TV_T^2}{(2R_L)^2}= \frac{R_TV_T^2}{4R_T^2}=\frac{V_T^2}{4R_T}
\end{equation}
}
